...
"""
    ...
    for issue in top_issues:
        violation_rows = issue.get("violation_rows") or []
        if violation_rows:
            total_count = len(violation_rows)
            preview = ", ".join(str(v) for v in violation_rows[:10])
            if total_count > 10:
                rows_text = f"{preview} ... (total {total_count})"
            else:
                rows_text = preview
        else:
            rows_text = "-"
  ...
  <table>
    ...
    <tbody>
"""
    top_table += "\n".join(top_rows) if top_rows else "<tr><td colspan=\"8\">No issues</td></tr>"
    top_table += """
    </tbody>
  </table>
"""
    ...
    for r in results:
        ...
        violation_rows = r.get("violation_rows") or []
        if violation_rows:
            rows_text = ", ".join(str(v) for v in violation_rows)
        else:
            rows_text = "-"
        result_rows.append(
            "<tr>"
  ...
  <table>
    ...
    <tbody>
"""
    results_table += "\n".join(result_rows) if result_rows else "<tr><td colspan=\"12\">No results</td></tr>"
    results_table += """
    </tbody>
  </table>
"""
