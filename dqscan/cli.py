...
def main(argv: list[str] | None = None) -> int:
    ...
    # Always honor the --input path; do not override based on NL prompt contents.
    ...
    if nl_text:
        ...
        for msg in pruned_warnings:
            print(f"- rule_pruned: {msg}")
        rules_data, auto_warnings = add_missing_rules_for_domain(rules_data, max_missing_rate=0.0)
        for msg in auto_warnings:
            print(f"- rule_added: {msg}")
    ...
    top_issues = None
    outputs_cfg = expanded_rules_data.get("outputs")
    if isinstance(outputs_cfg, dict) and "top_issues" in outputs_cfg:
        top_issues = outputs_cfg.get("top_issues")
    report = run_scan(
        df=df,
